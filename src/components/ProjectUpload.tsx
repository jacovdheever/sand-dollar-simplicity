...
import { X, Upload, Image, FileText, Calendar, User, Clock, Tag } from 'lucide-react';
...
const ProjectUpload: React.FC<ProjectUploadProps> = ({ onProjectCreated, onClose }) => {
  ...
  const handleContentImagesChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (files.length > 0) {
      ...
      setContentImagePreviews(processedImages);
    }
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // Generate image URL (in a real app, you'd upload to a server)
      ...
      <div className="flex min-h-full items-center justify-center p-4">
        <div className="relative w-full max-w-4xl bg-white rounded-2xl shadow-2xl max-h-[90vh] overflow-hidden">
          <div className="sticky top-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
            <h2 className="text-xl font-bold text-gray-900">Upload New Project</h2>
            <button
              onClick={onClose}
